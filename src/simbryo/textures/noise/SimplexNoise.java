 ...
 * 
 * Found at: http://staffwww.itn.liu.se/~stegu/simplexnoise/SimplexNoise.java
 *
 * @author royer
 */
public class SimplexNoise extends TextureGeneratorBase
                          implements TextureGeneratorInterface
{

  private short[] mPermutation;
  ...
  public SimplexNoise(int pDimension)
  {
    ...
    mPermutation = new short[256];

    for (int i = 0; i < mPermutation.length; i++)
      mPermutation[i] = (short) i;

    for (int i = 0; i < mPermutation.length; i++)
    {
      int u = (int) Math.floor(Math.random() * (mPermutation.length));

      short lTemp = mPermutation[i];
      mPermutation[i] = mPermutation[u];
      mPermutation[u] = lTemp;
    }

    for (int i = 0; i < 512; i++)
    {
      perm[i] = mPermutation[i & 255];
      permMod12[i] = (short) (perm[i] % 12);
    }

    System.out.println(Arrays.toString(mPermutation));
  ...
  {
    SimplexNoise lSimplexNoise = new SimplexNoise(getDimension());
  ...
  private static double dot(Grad g,
                            double x,
                            double y,
                            double z,
                            double w)
  ...
  private double noise(double x, double y, double z, double w)
  {
    ...
    // [rankx, ranky, rankz, rankw] is a 4-vector with the numbers 0, 1, 2 and
    // 3
    ...
    double x3 = x0 - i3 + 3.0 * G4; // Offsets for fourth corner in (x,y,z,w)
                                    // coords
